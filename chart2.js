class Chart2 {
  ...
  setup(rawData) {
    ...
    let maxDeces = Math.max(...this.data.flat().map(d => d.Deces));
    let maxInfection = Math.max(...this.data.flat().map(d => d.Infection));
  ...
  update(day) {
    d3.selectAll(".chart2 .day")
      .data([this.data[day]])
      .text((d) => d.date.format("DD/MM/YYYY"));

    d3.selectAll(".chart2 .circle")
  ...
  deces(datum) {
    ...
    let v = this.x(datum.datum.Deces);
    return v;
  }

  infection(datum) {
    let p = this.pays.find((p) => p.name == datum.name);
    let v = this.y(datum.datum.Infection);
    return v;
  }
}
